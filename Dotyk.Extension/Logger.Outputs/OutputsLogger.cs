...
using Dotyk.Extension.Extensions.Logging;
using Dotyk.Extension.Logging.Outputs.Internal;
using Dotyk.Extension.Extensions.Logging.Outputs.Internal;
using Microsoft.Extensions.Logging.Abstractions.Internal;


namespace Dotyk.Extension.Logging.Outputs
{
    public class OutputsLogger : ILogger
    {
        ...
        private readonly OutputsLoggerProcessor _queueProcessor;
        private Func<string, LogLevel, bool> _filter;

        [ThreadStatic]
        private static StringBuilder _logBuilder;

        static OutputsLogger()
        {
        ...
        public OutputsLogger(string name, Func<string, LogLevel, bool> filter, bool includeScopes)
            : this(name, filter, includeScopes ? new LoggerExternalScopeProvider() : null, new OutputsLoggerProcessor())
        {
        }

        public OutputsLogger(string name, Func<string, LogLevel, bool> filter, IExternalScopeProvider scopeProvider)
            : this(name, filter, scopeProvider, new OutputsLoggerProcessor())
        {
        }

        internal OutputsLogger(string name, Func<string, LogLevel, bool> filter, IExternalScopeProvider scopeProvider, OutputsLoggerProcessor loggerProcessor)
        {
            if (name == null)
            {
        ...
        private void GetScopeInformation(StringBuilder stringBuilder)
        {
            ...
            if (scopeProvider != null)
            {
                ...
                }, (stringBuilder, initialLength)); // error is appearing when  target framework lower then 4.7.1
