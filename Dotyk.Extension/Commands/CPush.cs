...
using PushOption = Dotyk.Store.Cli.PushOption;
...
namespace Dotyk.Extension.Commands {
    ...
    internal sealed class CPush
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CPush"/> class.
        /// Adds our command handlers for menu (commands must exist in the command table file)
        /// </summary>
        /// <param name="package">Owner package, not null.</param>
        /// <param name="commandService">Command service to add command to, not null.</param>
        private CPush(AsyncPackage package, OleMenuCommandService commandService)
        {
        ...
        {
            // Verify the current thread is the UI thread - the call to AddCommand in CPush's constructor requires
            // the UI thread.
            ThreadHelper.ThrowIfNotOnUIThread();

            OleMenuCommandService commandService = await package.GetServiceAsync((typeof(IMenuCommandService))) as OleMenuCommandService;
            Instance = new CPush(package, commandService);
        }
